...
public:
   ...
   * @return {*}
   */
  virtual void init(const Algo_Type algo_type, const Device_Type device_type,
   ...
   * @return {*}
   */
  void infer(const std::string file_path, bool save_result = true,
             bool show_result = true, char *argv[] = {});

  /**
   * @description:                inference interface for cv::Mat
   * @param {cv::Mat&} image      input image
   * @return {*}
   */
  void infer_image(const cv::Mat &image);
...
public:
   ...
   * @return {*}
   */
  void register_class(const Backend_Type &backend_type,
